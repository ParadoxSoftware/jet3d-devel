...
jeBoolean Util::IsKeyDown( int vKey )
{
	short KeyState;
...
jeBoolean Util::Polyline( int32 hDC, Point * pPoints, int32 nPoints )
{
	return ::Polyline( (HDC)hDC, (POINT*)pPoints, nPoints ) ;
}// Util_Polyline

jeFloat Util::PointToLineDistanceSquared( const Point * pL1, const Point * pL2, const Point * pPoint )
{
	int32	xkj, ykj ;
	jeFloat	t ;
	jeFloat	xfac, yfac ;
...
jeFloat	Util::PointDistanceSquared( const Point * pL1, const Point * pL2 )
{
	int32 dX, dY;
...
void Util::ExtBox_Union( const jeExtBox *B1, const jeExtBox *B2, jeExtBox *Result )
{
	assert( B1 != NULL ) ;
	assert( B2 != NULL ) ;
	assert( Result != NULL );
...
void Util::ExtBox_ExtendToEnclose( jeExtBox *B, const jeVec3d *Point )
{
	assert( B != NULL ) ;
	assert( Point != NULL );
...
jeBoolean Util::ExtBox_Intersection ( const jeExtBox *B1, const jeExtBox *B2, jeExtBox *Result	)
{
...
void Util::ExtBox_InitFromTwoPoints( jeExtBox * B, const jeVec3d * p1, const jeVec3d * p2 )
{
	assert( B != NULL ) ;
	assert( p1 != NULL ) ;
	assert( p2 != NULL ) ;
...
jeFloat Util::ExtBox_GetExtent( const jeExtBox *B, int32 nElement )
{
	assert( B != NULL ) ;
	assert( nElement < 3 ) ;
...
void Util::ExtBox_SetInvalid( jeExtBox *B )
{
	B->Min.X = B->Min.Y = B->Min.Z = FLT_MAX ;
	B->Max.X = B->Max.Y = B->Max.Z = -FLT_MAX ;
}// Util_ExtBox_SetInvalid

void Util::ExtBox_Transform( const jeExtBox *B, const jeXForm3d *XForm, jeExtBox *Result )
{
...
void Util::ExtBox_TransformJ( const jeExtBox *B, const jeXForm3d *XForm, jeExtBox *Result )
{
	jeVec3d				Verts[8] ;
	int					i ;
...
jeFloat Util::log2( jeFloat f )
{
	return (jeFloat)(log (f)/log (2.0f));
}// log2

jeFloat Util::NearestLowerPowerOf2( const jeFloat fVal )
{
	// This would likely be faster if you count the bits
	// and then loop multiplying by 2 for each bit
	return (jeFloat)::pow(2, log2(fVal) );
}// Util_NearestLowerPowerOf2


void Util::DriveAndPathOnly( char * pszPath ) 
{
...
void Util::NameOnly( char * pszPath )
{
...
void Util::StripTrailingBackslash(char * pszString )
{
	int len;
...
void Util::NewExtension(char * pszString, const char * pszNewExt)
{
...
jeBoolean Util::VFile_ReadString( jeVFile * pFile, char * pszBuffer, const int32 nMaxChars )
{
	int32		i ;
...
////////////////////////////////////////////////////////////////////////////////////////
int Util::GetAppPath(
	wchar_t	*Buf,		// where to store path name
