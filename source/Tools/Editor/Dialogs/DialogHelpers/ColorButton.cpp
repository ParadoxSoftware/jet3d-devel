...
void AFXAPI DDX_ColorButton(CDataExchange *pDX, int nIDC, COLORREF& crColor)
{
    ...
    CColorButton* pColorButton = (CColorButton*) CWnd::FromHandle(hWndCtrl);
    if (pDX->m_bSaveAndValidate)
    {
		crColor = pColorButton->Color;
    }
    else // initializing
    {
		pColorButton->Color = crColor;
    }
}
...
LONG CColorButton::OnSelChange(UINT lParam, LONG wParam)
{
    ...
    if (pParent) pParent->SendMessage(CPN_SELCHANGE, (LONG) GetDlgCtrlID(), lParam );
