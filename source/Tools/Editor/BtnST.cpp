...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
	...
	//	by trilobite	Jan. 2011
	CMemDC pMemDC(*CDC::FromHandle(lpDIS->hDC), &totalAreaWnd);
...
//	tom morris feb 2005 -- condition commented out
//  if (m_ViewBuffer.IsDirty())		//	If it's empty or uninitialized, let's draw to it
  ...
  if (bIsPressed)
  {
    if (m_bIsFlat == TRUE)
    {
      ...
      {
		  //	by trilobite	Jan. 2011
		  //pMemDC->Draw3dRect(itemRect, ::GetSysColor(COLOR_BTNSHADOW), ::GetSysColor(COLOR_BTNHILIGHT));
    ...
    else    
    {
	  ...
	  //	by trilobite	Jan. 2011
      //pMemDC->FrameRect(&itemRect, &brBtnShadow);
  ...
  else // ...else draw non pressed button
  {
    ...
    if (m_bIsFlat == TRUE)
    {
      ...
      {
		  //	by trilobite	Jan. 2011
		  //pMemDC->Draw3dRect(itemRect, ::GetSysColor(COLOR_BTNHILIGHT), ::GetSysColor(COLOR_BTNSHADOW));
...
/*
      ...
      // White line
      pOldPen = pMemDC->SelectObject(&penBtnHiLight);
      pMemDC->MoveTo(itemRect.left, itemRect.bottom-1);
      pMemDC->LineTo(itemRect.left, itemRect.top);
      pMemDC->LineTo(itemRect.right, itemRect.top);
      // Light gray line
      pMemDC->SelectObject(pen3DLight);
      pMemDC->MoveTo(itemRect.left+1, itemRect.bottom-1);
      pMemDC->LineTo(itemRect.left+1, itemRect.top+1);
      pMemDC->LineTo(itemRect.right, itemRect.top+1);
      // Disegno i bordi a destra e in basso
      // Black line
      pMemDC->SelectObject(pen3DDKShadow);
      pMemDC->MoveTo(itemRect.left, itemRect.bottom-1);
      pMemDC->LineTo(itemRect.right-1, itemRect.bottom-1);
      pMemDC->LineTo(itemRect.right-1, itemRect.top-1);
      // Dark gray line
      pMemDC->SelectObject(penBtnShadow);
      pMemDC->MoveTo(itemRect.left+1, itemRect.bottom-2);
      pMemDC->LineTo(itemRect.right-2, itemRect.bottom-2);
      pMemDC->LineTo(itemRect.right-2, itemRect.top);
      //
...
//
  ...
  {
	  //	by trilobite	Jan. 2011
    //DrawTheIcon(pMemDC, &sTitle, &lpDIS->rcItem, &captionRect, bIsPressed, bIsDisabled);
 ...
 //	by trilobite	Jan. 2011. replacing pMemDC with pDC below 

	// Get dialog's font
    CFont *pCurrentFont = GetFont(); 
    //CFont *pOldFont = pMemDC->SelectObject(pCurrentFont);
    CFont *pOldFont = pDC->SelectObject(pCurrentFont);

	if ((m_MouseOnButton == TRUE) || (bIsPressed)) 
	{
      //pMemDC->SetTextColor(GetActiveFgColor());
      //pMemDC->SetBkColor(GetActiveBgColor());
      pDC->SetTextColor(GetActiveFgColor());
      pDC->SetBkColor(GetActiveBgColor());
	} 
	else 
	{
      //pMemDC->SetTextColor(GetInactiveFgColor());
      //pMemDC->SetBkColor(GetInactiveBgColor());
      pDC->SetTextColor(GetInactiveFgColor());
      pDC->SetBkColor(GetInactiveBgColor());
    }
    // Center text
    CRect centerRect = captionRect;
    //pMemDC->DrawText(sTitle, -1, captionRect, DT_SINGLELINE|DT_CALCRECT);
    pDC->DrawText(sTitle, -1, captionRect, DT_SINGLELINE|DT_CALCRECT);
    captionRect.OffsetRect((centerRect.Width() - captionRect.Width())/2, (centerRect.Height() - captionRect.Height())/2);
	
	//pMemDC->SetBkMode(TRANSPARENT);
    //pMemDC->DrawState(captionRect.TopLeft(), captionRect.Size(), (LPCTSTR)sTitle, (bIsDisabled ? DSS_DISABLED : DSS_NORMAL), 
    //               TRUE, 0, (CBrush*)NULL);
