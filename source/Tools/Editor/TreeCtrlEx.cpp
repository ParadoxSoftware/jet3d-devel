...
void CTreeCtrlEx::OnLButtonDown( UINT nFlags, CPoint point )
{
	...
	if( nHitFlags & TVHT_ONITEMLABEL )
	{
		...
		{
			if( m_nFlags & CTreeCtrlEx::NOROOTEDIT )
			{
				if( GetParentItem( hClickedItem ) != NULL )	// NOT a root item
				{
...
void CTreeCtrlEx::SelectMultiple( HTREEITEM hClickedItem, UINT nFlags )
{
	...
	if ( pWnd )
	{
		...
		tv.action = TVC_BYCTREECTRL /*TVC_UNKNOWN added by jeff */ ;
...
void CTreeCtrlEx::SetFlags(int nFlags)
{
	m_nFlags = nFlags ;
}
