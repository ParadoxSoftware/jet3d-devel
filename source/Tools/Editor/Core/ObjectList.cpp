...
ObjectIterator ObjectList_Find( ObjectList * pList, Object * pObject )
{
	...
	List_Search( pList, ObjectList_FindCB, pObject, (void**)&pFoundObject, &pBI ) ;
	return pBI ;
...
jeBoolean ObjectList_AppendNoDup( ObjectList * pList, Object * pObject )
{
	...
	bFound = List_Search( pList, ObjectList_FindCB, pObject, (void**)&pFoundObject, &pBI ) ;
	if( bFound == JE_FALSE )
	{
...
ObjectIterator ObjectList_AppendSort( ObjectList * pList, Object * pObject, ObjectList_SortCB Callback)
{
	...
	pObj = (Object*)List_GetFirst(pList, &it);
	while (pObj) {
		if (Callback(pObject, pObj)) {
			break;
		}
		pObj = (Object*)List_GetNext(pList, &it);
	}
	if (it) {
...
{
	Object * pFreeObject = (Object*)pObject ;
	assert( pObject != NULL ) ;
	
	Object_Free( &pFreeObject ) ;
}//ObjectList_DestroyCB

static jeBoolean ObjectList_UnionObjectRectCB( Object * pObject, void * pListBounds )
{

	jeExtBox  ObjectBounds;

	if( Object_GetWorldAxialBounds(pObject, &ObjectBounds) )
		Util_ExtBox_Union( (jeExtBox*)pListBounds, &ObjectBounds, (jeExtBox*)pListBounds ) ;
	return JE_TRUE ;
...
static jeBoolean ObjectList_UnionObjectDrawRectCB( Object * pObject, void * pListBounds )
{
