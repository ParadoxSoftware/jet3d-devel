...
List * pNew;
	...
	pNew = (List *)MemPool_GetHunk(ListPool_g);
	assert(pNew);
	pNew->Data = 0;
...
List *	LISTCALL List_AddTail(List *pList,uint32  Data)
{
List * pNew;
...
List *	LISTCALL List_AddHead(List *pList,uint32  Data)
{
List * pNew;
...
uint32 	LISTCALL List_CutHead(List *pList)
{
List * pNode;
uint32  pData;
	pNode = pList->Next;
	if ( pNode == pList ) return 0;
	pData = pNode->Data;
...
uint32 	LISTCALL List_CutTail(List *pList)
{
List * pNode;
uint32  pData;
	pNode = pList->Prev;
...
uint32 	LISTCALL List_PeekHead(List *pList)
{
List * pNode;
	pNode = pList->Next;
	if ( pNode == pList ) return 0;
...
uint32 	LISTCALL List_PeekTail(List *pList)
{
List * pNode;
	pNode = pList->Prev;
	if ( pNode == pList ) return 0;
...
uint32 	LISTCALL List_NodeData(List *pNode)
{
	assert(pNode);
	return pNode->Data;
}
...
List *	List_Find(List *pList,uint32 Data)
{
List *pNode;
	assert(pList);
...
int		LISTCALL List_Length(List *pList)
{
int Len = 0;
List *pNode;
	assert(pList);
...
{
Link * pLink;
	pLink = (Link *)MemPool_GetHunk(LinkPool_g);
	assert(pLink);
	pLink->Next = NULL;
	pLink->Data = NULL;
return pLink;
...
Link *	LISTCALL Link_Next(Link *pLink)
{
	if ( ! pLink ) return NULL;
return pLink->Next;
}

void *	LISTCALL Link_Data(Link *pLink)
{
	if ( ! pLink ) return NULL;
return pLink->Data;
}
...
Stack * pStack;
	...
	assert(pStack);
	pStack->members = 4096;
	pStack->Buffer = (void **)MemAlloc(sizeof(void *)*(pStack->members));
...
void LISTCALL Stack_Reset(Stack * pStack)
{
	pStack->End = pStack->Buffer + (pStack->members);
	pStack->Head = pStack->Buffer;
}
...
int newmembers;
	...
	NewBuffer = (void **)MemAlloc(sizeof(void *)*newmembers);
	assert(NewBuffer);
...
void	LISTCALL Stack_PushData(Stack *pStack,void *Data,int DataLen)
{
	DataLen = (DataLen+3)>>2;
	
	if ( (pStack->Head+DataLen) >= pStack->End )
		Stack_Extend(pStack);
		
	memcpy(pStack->Head,Data,DataLen<<2);
	pStack->Head += DataLen;
}

int		LISTCALL Stack_PopData( Stack *pStack,void *Data,int DataLen)
{
	assert(pStack);
	...
	pStack->Head -= DataLen;
	memcpy(Data,pStack->Head,DataLen<<2);
	return 1;
}
...
int r;
	pRadixList = (RadixList *)new(RadixList);
	assert(pRadixList);
	pRadixList->NumLists = RadixListMax+1;
	pRadixList->Lists = (List **)MemAlloc(sizeof(List *)*(pRadixList->NumLists));
...
List * RadixList_Add(RadixList *pRadixList,uint32  Data,int Key)
{
List * l;
	assert(pRadixList);
...
uint32  RadixList_CutMax(RadixList *pRadixList,int *pKey)
{
	assert(pRadixList);
...
uint32  RadixList_CutMin(RadixList *pRadixList,int *pKey)
{
	assert(pRadixList);
...
uint32  RadixList_CutKey(RadixList *pRadixList,int Key)
{
	assert(pRadixList);
...
RadixLN * pRadixLN;
	...
	pRadixLN = (RadixLN *)new(RadixLN);
	assert(pRadixLN);

	pRadixLN->NumLNs = RadixLNMax+1;

	pRadixLN->LNs = (LinkNode *)MemAlloc(sizeof(LinkNode)*(pRadixLN->NumLNs));

	assert(pRadixLN->LNs);

	RadixLN_Reset(pRadixLN);

return pRadixLN;
}

void RadixLN_Reset(RadixLN * pRadixLN)
{
LinkNode * LNs;
int r;
	assert(pRadixLN->LNs);
...
TIMER_P(List_RadixInit); // not counting the allocs, tracking in List_Ram
	...
	pRadixLN->Min = pRadixLN->NumLNs-1;
	pRadixLN->Max = 0;
...
RadixLink * pRadixLink;
	pRadixLink = (RadixLink *)new(RadixLink);
	assert(pRadixLink);
	pRadixLink->NumLinks = RadixLinkMax+1;
	pRadixLink->Links = (Link *)MemAlloc(sizeof(Link)*(pRadixLink->NumLinks));
...
HashNode *pHead,*pTail;
	...
	pTail = (HashNode *)MemPool_GetHunk(HashNodePool_g);
	assert(pTail);
	LN_Null(pTail);
	pTail->Key = pTail->Data = 0;
	pTail->Hash = HASH_SIZE;

	pHead = (HashNode *)MemPool_GetHunk(HashNodePool_g);
	assert(pHead);
	LN_Null(pHead);
...
Debug( int ListLen1; int ListLen2; int HashLen1; int HashLen2; int WalkLen)
	...
	hn = (HashNode *)MemPool_GetHunk(HashNodePool_g);
	assert(hn);
	Debug( LN_Null(hn) )
