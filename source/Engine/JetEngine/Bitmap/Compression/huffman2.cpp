 ...
 *
 * this can mess you up in merge-bit-stream cases
 *
 *
...
void Huff2_CleanUp(struct Huff2Info * HI)
{
FreeMem(HI->NodeWork,sizeofpointer*HI->NumSymbols);
FreeMem(HI->MadeNodeWork,sizeofpointer*HI->NumSymbols*2);
FreeMem(HI->CodeLenTable,HI->NumSymbols*sizeof(long));
FreeMem(HI->NumCodesOfLen,32*sizeof(long));
FreeMem(HI->CodePrefixByLen,32*sizeof(uint32));
FreeMem(HI->CodeNodeHunk,sizeof(struct Huff2CodeNode)*2*HI->NumSymbols);
FreeMem(HI->StackArray,2*(HI->NumSymbols)*sizeof(uint32));
FreeMem(HI->EnDe_codeTable,HI->EnDe_codeTableLen);
...
while( NodeWorkOut < (GotNumSymbols-1) ) /* $ 38 % */
	...
	if ( HI->CodeNodeHunkI >= (NumSymbols*2) )
		{
		BrandoError(" HI->CodeNodeHunkI >= (NumSymbols*2) !");	
		exit(1);
		}
#endif // DEBUG
...
/* now read out the codelens into an array */
  {
	...
	long * NumCodesOfLen;
	register struct Huff2CodeNode * CurNode;
	register uint32 CurCodeLen;
	register uint32 GotC;
	register uint32 * StackArray;
	...
	StackArray[0] = (uint32) HI->NodeBase;
	StackArray[1] = 0;
	StackI = 2;
	while ( StackI > 0 ) /* $ 15% */
		{
		...
		while ( CurNode )
			...
			if ( GotC == HUFF2_CODE_BRANCH )
				...
				if ( CurNode->Down )
					...
					if ( GotC == HUFF2_CODE_BRANCH )
						{
						StackArray[StackI++] = (uint32) CurNode->Down;
						StackArray[StackI++] = CurCodeLen;
	
#ifdef DEBUG
						if ( StackI >= (HI->NumSymbols * 2) )
							{
							BrandoError("Stack usage higher than expected!");
							exit(10);
							}
#endif
...
if ( HI->EnDe_codeTable )
	...
	HI->EnDe_codeTableLen = NumSymbols*sizeof(uint32);
	}
...
long NumSymbols;
long Huff2CodeLen;
uint32 CurCode,StartCode,Huff2Code,CheckCode,ReadCodeMask,PackedCode;
...
DecodeTableLen = 2*NumSymbols*sizeof(uint16);
...
void Huff2_EncodeC(struct Huff2Info *HI,uint16 Symbol)
{
...
CurCode 	 = ((uint32 *)HI->EnDe_codeTable)[Symbol];
...
if ( CurCodeLen == 0 )
	{
	BrandoError("Got EncodeC for CodeLen == 0 symbol!");
	exit(10);
	}
#endif
...
NumSymbols = HI->NumSymbols;
FastDecodeTable = (struct FastDecodeItem *) ( ((char *)HI->EnDe_codeTable) + 2*NumSymbols*sizeof(uint16) );
DecodeTable = (uint16 *)HI->EnDe_codeTable;
CodePrefixByLen = HI->CodePrefixByLen;
CurArrayPtr = Array;
...
if ( FastDecodeTable[PeekedCode].NumCharsMade > 0 )
	...
	if ( CurCodeLen > HI->MaxCodeLen )
		{
		BrandoError("Got code longer than MaxCodeLen!");
		exit(10);
		}
#endif
...
memclearFast(HI->CodeLenTable,HI->NumSymbols);
memclearFast(HI->NumCodesOfLen,32);
HI->MinCodeLen = HI->MaxCodeLen = 0;
...
long MinCodeLen,MaxCodeLen;
long i,j,GotNumSymbols;
uint8 RunData[512],RunDataLast[512];
long RunLen;
long CurCodeLen;
struct LBitIOInfo * BII;
